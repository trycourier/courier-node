...
describe("HttpClient", () => {
  ...
  test("should URI encode values", async () => {
    ...
    await client.put(
      "https://api.courier.com/lists/list-1/subscriptions/subscriber|some-value+other-value?query=param"
    );
    expect(fetchMock.mock.calls.length).toEqual(1);
    expect(fetchMock.mock.calls[0][0]).toEqual(
      "https://api.courier.com/lists/list-1/subscriptions/subscriber%7Csome-value+other-value?query=param"
    );
  });

  test("should prepend baseURL", async () => {
    const client = initHttpClient({
      baseUrl: "https://execute-api.us-east-1.amazonaws.com/dev",
      version: "1",
      authorizationToken: "auth-token"
    });
    ...
    expect(fetchMock.mock.calls.length).toEqual(1);
    expect(fetchMock.mock.calls[0][0]).toEqual(
      "https://execute-api.us-east-1.amazonaws.com/dev/lists/list1%7Ctest.list"
    );
  });
});
