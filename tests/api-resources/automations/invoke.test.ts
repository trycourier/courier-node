...
describe('resource invoke', () => {
  ...
  test.skip('invokeAdHoc: only required params', async () => {
    const responsePromise = client.automations.invoke.invokeAdHoc({ automation: { steps: [{ action: 'delay' }, { action: 'send' }] } });
  ...
  test.skip('invokeByTemplate: only required params', async () => {
    const responsePromise = client.automations.invoke.invokeByTemplate('templateId', { recipient: 'recipient' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
