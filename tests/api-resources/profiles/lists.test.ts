...
describe('resource lists', () => {
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.profiles.lists.retrieve('user_id', { cursor: 'cursor' }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('subscribe: required and optional params', async () => {
    const response = await client.profiles.lists.subscribe('user_id', { lists: [{
    listId: 'listId',
    preferences: {
    categories: { foo: {
    status: 'OPTED_IN',
