...
describe('resource routingStrategies', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.routingStrategies.list({ cursor: 'cursor', limit: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('listNotifications: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.routingStrategies.listNotifications('id', { cursor: 'cursor', limit: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Courier.NotFoundError);
  });
