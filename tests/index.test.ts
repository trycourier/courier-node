...
import { APIPromise } from '@trycourier/courier/core/api-promise';

import util from 'node:util';
import Courier from '@trycourier/courier';
import { APIUserAbortError } from '@trycourier/courier';
...
describe('logging', () => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Courier({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'COURIER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['COURIER_API_KEY'] = 'My API Key';
    const client = new Courier();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['COURIER_API_KEY'] = 'another My API Key';
    const client = new Courier({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
